 ...
 * As funções abaixo, fornece suporte ao conforto visual aplicado aos menus e tabs.
 * A segurança não é feita via javascript, mas sim no servidor.
 */
...
seg.security = function(){
}

/** Key dos dados de segurança armazenados em sessionStorage. */
seg.security.SESSION_STORAGE_KEY = "plc.security.data";

/** Prototypo da classe seg.Security */
seg.security.prototype = {
	...
	,getSecurityData: function( avoidSessionStore ) {
		...
		if ( !this.securityData ) {
			...
			if ( avoidSessionStore !== true && typeof sessionStorage != "undefined" && sessionStorage.length > 0 ) {
				...
				var sessionData = sessionStorage.getItem( seg.security.SESSION_STORAGE_KEY );
	...
	,setSecurityData: function( data, avoidSessionStore ) {
		...
		data = plc.jq.trim( data );
		
		if ( data && data.charAt(0) == '{' && data.charAt(data.length - 1) == '}' ) {
			if ( typeof sessionStorage != "undefined" && avoidSessionStore !== true ) {
				// recupera o dado armazenado no sessionStorage.
				sessionStorage.setItem( seg.security.SESSION_STORAGE_KEY, data );
			}
			this.securityData = plc.jq.parseJSON(data).securityApp || null;
		} else {
	...
	,getCurrentSession: function(){
		return plc.jq.cookie("JSESSIONID");
	}
	...
	,load: function( callback ){
		...
		if ( securityData && securityData.session == currentSession ) {
			...
			plc.jq.ajax({
				url: plcGeral.contextPath + "/soa/service/securityApp"
				,type: "GET"
				,dataType: "text"
				,context: this
...
seg.security.load = function( callback ) {
